...
Dense::Dense(size_t input_size, size_t output_size)
    : weights{{input_size, output_size}}, biases{{1, output_size}}, grad_weights{{input_size, output_size}}, grad_biases{{1, output_size}}
{
...
Tensor Dense::forward(const Tensor &input)
{
    ...
    try
    {
        if(backendType == Backend::GPU)
        {
            ...
            if(!input.isOnGpu()) { const_cast<Tensor &>(input).toGpu(); }

            if(!weights.isOnGpu()) { weights.toGpu(); }

            if(!biases.isOnGpu()) { biases.toGpu(); }
    ...
    {
        std::cerr << "[APP_LOG] Error during forward pass (GPU): " << e.what() << ". Falling back to CPU implementation." << '\n';
        std::cout << "[APP_LOG] GPU forward failed in Dense: " << e.what() << ". Falling back to CPU." << '\n';
...
Tensor Dense::backward(const Tensor &grad_output)
{
    ...
    try
    {
        ...
        {
            // Ensure tensors are on GPU
            if(!last_input_T.isOnGpu()) { last_input_T.toGpu(); }

            if(!grad_output.isOnGpu()) { const_cast<Tensor &>(grad_output).toGpu(); }

            if(!weights_T.isOnGpu()) { weights_T.toGpu(); }
    ...
    {
        std::cerr << "[APP_LOG] Error during backward pass (GPU): " << e.what() << ". Falling back to CPU implementation." << '\n';
        std::cout << "[APP_LOG] GPU backward failed in Dense: " << e.what() << ". Falling back to CPU." << '\n';
...
void Dense::update(Optimizer &optimizer)
{
    ...
    // Ensure GPU copies are refreshed on next forward after CPU updates
    if(backendType == Backend::GPU)
    {
        weights.freeGpu();
        biases.freeGpu();
