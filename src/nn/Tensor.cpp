...
Tensor::Tensor(const std::vector<size_t> &shape) : shape{shape}, totalSize{0}, cpu_data{nullptr}, gpu_data{nullptr}
{
    calculateSize();
    allocateCpu();
}
...
void Tensor::calculateSize()
{
    ...
    totalSize = std::accumulate(shape.begin(), shape.end(), static_cast<size_t>(1), std::multiplies<size_t>());
}
...
#else
        std::cerr << "Warning: Attempting to copy a GPU tensor in a non-CUDA build." << '\n';
#endif
    }
}
...
Tensor &Tensor::operator=(const Tensor &other)
{
    ...
    if(other.gpu_data)
    {
        ...
        cudaMemcpy(gpu_data, other.gpu_data, totalSize * sizeof(float), cudaMemcpyDeviceToDevice);
#else
        std::cerr << "Warning: Attempting to assign a GPU tensor in a non-CUDA build." << '\n';
#endif
    }
    return *this;
}
...
Tensor::Tensor(Tensor &&other) noexcept : shape{std::move(other.shape)}, totalSize{other.totalSize}, cpu_data{other.cpu_data}, gpu_data{other.gpu_data}
{
...
{
    const size_t newSize = std::accumulate(newShape.begin(), newShape.end(), static_cast<size_t>(1), std::multiplies<size_t>());
...
Tensor Tensor::outer(const Tensor &v1, const Tensor &v2)
{
    ...
    const size_t n1 = v1.getCols();
    const size_t n2 = v2.getCols();
    Tensor result{{n1, n2}};
